    ...
    <div className="pt-20 pb-12 px-4 sm:px-8 max-w-7xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-2xl font-black text-slate-900 tracking-tight">Live Emergency Delivery & Cold-Chain Logistics</h2>
            <span className="text-xs font-mono font-bold bg-sky-50 text-sky-700 border border-sky-200 px-2 py-0.5 rounded-full">
      ...
      {currentReq ? (
        <div className="space-y-6">
          ...
          <div className="glass-panel bg-white border border-slate-200 p-6 rounded-3xl grid grid-cols-1 sm:grid-cols-4 gap-4 text-xs shadow-sm">
            <div>
              <span className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Tracking Requisition ID</span>
              <span className="font-mono font-black text-slate-900 text-base">{currentReq.id}</span>
            ...
            <div>
              ...
              <span className="font-bold text-slate-800">{currentReq.patientName}</span>
              <span className="text-[11px] text-slate-500 block mt-0.5">{currentReq.contactPhone}</span>
            </div>

            <div>
              <span className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Destination Trauma Center</span>
              <span className="font-bold text-rose-600">{currentReq.hospitalName}</span>
              <span className="text-[10px] text-slate-400 font-mono block mt-0.5">ICU Acute Trauma Unit</span>
            </div>

            <div>
              <span className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Payload Specification</span>
              <div className="flex items-baseline gap-2 mt-0.5">
                <span className="text-base font-bold font-mono text-white bg-rose-600 px-2 py-0.5 rounded-lg shadow-sm">
                  {currentReq.bloodType}
                </span>
                <span className="text-xs text-slate-700 font-mono font-bold">{currentReq.unitsRequired || 2} Units Required</span>
              </div>
            </div>
          </div>

          {/* 5-Step Delivery Milestone Timeline */}
          <div className="glass-panel bg-white border border-slate-200 p-6 rounded-3xl shadow-sm">
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-6 flex items-center justify-between">
              ...
              {STEPS.map((step, idx) => {
                ...
                return (
                  <div key={idx} className="relative flex items-start justify-between gap-4">
                    <div
                      className={`absolute -left-6 w-5 h-5 rounded-full border-2 flex items-center justify-center text-[10px] font-bold transition-all ${
                        isDone
                          ...
                          : isCurrent
                          ? 'bg-rose-600 border-rose-400 text-white animate-pulse shadow-sm'
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className={`text-xs font-bold ${isDone ? 'text-slate-900' : 'text-slate-500'}`}>
                          {step.title}
                        </h4>
                        {isCurrent && (
                          <span className="text-[9px] font-mono bg-rose-50 text-rose-600 border border-rose-200 px-1.5 py-0.2 rounded font-bold animate-pulse">
                            ACTIVE VECTOR
                          </span>
                        )}
                      </div>
          ...
          {relatedDispatch && (
            <div className="bg-white border border-sky-200 p-5 rounded-3xl flex flex-wrap items-center justify-between gap-4 shadow-sm">
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  ...
                  <div className="flex items-baseline gap-2">
                    <span className="text-2xl font-bold font-mono text-sky-700">{relatedDispatch.tempCelsius || 4.0}°C</span>
              ...
              <div className="flex items-center gap-6 text-xs font-mono text-slate-600">
                <div>
                  ...
                  <span className="font-bold text-slate-900">{relatedDispatch.speedMph || 48} mph</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 block">ALTITUDE</span>
                  <span className="font-bold text-slate-900">{relatedDispatch.altitudeMeters || 150}m</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 block">BATTERY</span>
                  <span className="font-bold text-emerald-600">{relatedDispatch.batteryPct || 98}%</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="py-16 text-center text-sm text-slate-500 glass-panel bg-white p-8 rounded-3xl shadow-sm border border-slate-200">
          <Package className="w-8 h-8 text-slate-400 mx-auto mb-2" />
          <p className="font-semibold text-slate-700">No active emergency delivery tickets found.</p>
