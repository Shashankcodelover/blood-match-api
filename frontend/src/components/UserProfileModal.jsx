      ...
      >
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-800">
          <div className="flex items-center gap-3.5">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-purple-600 to-indigo-600 flex items-center justify-center text-white text-lg font-bold shadow-lg shadow-purple-600/30">
              {user?.name?.charAt(0) || 'U'}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-lg font-bold text-white">{user?.name}</h3>
                <span className="text-[10px] font-mono font-bold bg-rose-500/20 text-rose-300 border border-rose-500/30 px-2 py-0.5 rounded-full uppercase">
                  {user?.role}
                </span>
                {user?.bloodType && (
          ...
          <button
            onClick={onLogout}
            className="flex items-center gap-1.5 bg-slate-900 hover:bg-red-950/40 text-slate-300 hover:text-red-300 border border-slate-800 hover:border-red-500/40 text-xs font-semibold px-3 py-1.5 rounded-xl transition-all"
          >
        ...
        <form onSubmit={handleUpdateProfile} className="space-y-4 mb-6">
          <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider">Personal & Clinical Details</h4>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white outline-none focus:border-purple-500/60"
                value={name}
          ...
          {user?.role === 'donor' && (
            <div>
              <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Emergency Dispatch Availability Status</label>
              <select
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-emerald-400 font-semibold outline-none"
          ...
          {user?.role === 'patient' && (
            <div className="space-y-3">
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Emergency Contact Person</label>
                <input
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Pre-Existing Medical Notes / Allergies</label>
                <textarea
                  rows="2"
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white outline-none resize-none"
                  value={medicalNotes}
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex items-center justify-center gap-1.5 bg-purple-600 hover:bg-purple-500 text-white font-bold py-2 px-4 rounded-xl text-xs shadow-md transition-all"
          >
        ...
        <div className="border-t border-slate-800 pt-5">
          <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-3">Security & Password</h4>

          {passMsg && (
            <div className={`mb-3 p-2.5 rounded-xl text-xs border ${
              passMsg.type === 'success' ? 'bg-emerald-950/40 border-emerald-500/40 text-emerald-300' : 'bg-rose-950/40 border-rose-500/40 text-rose-300'
            }`}>
              {passMsg.text}
            </div>
          )}
          ...
          <form onSubmit={handleChangePassword} className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
            ...
            <button
              type="submit"
              className="bg-slate-800 hover:bg-slate-700 text-slate-200 font-semibold py-1.5 px-3 rounded-xl text-xs border border-slate-700 transition-all"
            >
              Update Password
            </button>
          </form>
