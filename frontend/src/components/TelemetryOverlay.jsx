...
export function TelemetryOverlay({
  ...
  return (
    <div className="fixed bottom-6 left-[395px] z-30 w-full max-w-md glass-panel p-4 rounded-2xl shadow-2xl border border-slate-700/80 animate-fade-in">
      {/* Multi-Dispatch Tabs */}
      {activeList.length > 1 && (
        <div className="flex items-center gap-1.5 mb-3 overflow-x-auto pb-1.5 border-b border-slate-800">
          {activeList.map(disp => (
            <button
              key={disp.id}
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-slate-950/80 p-2.5 rounded-xl border border-slate-800/80">
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-2xl font-black text-white font-mono">{currentDispatch.etaMinutes}</span>
            <span className="text-xs text-slate-400 font-semibold">{isArrived ? 'DELIVERED' : 'min'}</span>
          </div>
        </div>
        ...
        <div className="bg-slate-950/80 p-2.5 rounded-xl border border-slate-800/80">
          ...
          <div className="flex items-baseline gap-1.5">
            <span className={`text-2xl font-black font-mono ${isColdSafe ? 'text-cyan-400' : 'text-amber-400'}`}>
      ...
      <div className="grid grid-cols-4 gap-1.5 bg-slate-900/60 p-2 rounded-xl border border-slate-800/60 mb-3 text-center font-mono text-[11px]">
        <div>
          <span className="text-[8px] text-slate-500 block">SPEED</span>
          <span className="text-slate-200 font-semibold">{currentDispatch.speedMph} mph</span>
        </div>
        <div>
          <span className="text-[8px] text-slate-500 block">ALTITUDE</span>
          <span className="text-slate-200 font-semibold">{currentDispatch.altitudeMeters} m</span>
        </div>
        <div>
          <span className="text-[8px] text-slate-500 block">BATTERY</span>
          <span className="text-emerald-400 font-semibold">{currentDispatch.batteryPct}%</span>
        </div>
        <div>
          <span className="text-[8px] text-slate-500 block">WEATHER</span>
          <span className="text-cyan-300 font-semibold text-[10px]">Optimal</span>
        </div>
      </div>
      ...
      <div className="text-xs text-slate-400 flex items-center justify-between">
        <div>
          <span>Route: <strong className="text-white">{currentDispatch.donorName}</strong></span>
          <span className="text-[10px] font-mono text-slate-500 block">→ {currentDispatch.hospitalName} ({currentDispatch.remainingMiles} mi left)</span>
        </div>
        ...
        {currentDispatch.intakeConfirmation && (
          <span className="text-[10px] font-mono bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 px-2 py-0.5 rounded-md font-semibold">
      ...
      {showIntakeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm" onClick={() => setShowIntakeModal(false)}>
          <div className="w-full max-w-sm glass-panel p-5 rounded-2xl shadow-2xl border border-emerald-500/40 relative" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-2 mb-3">
              <UserCheck className="w-5 h-5 text-emerald-400" />
              <h4 className="text-sm font-bold text-white">Hospital Intake Verification</h4>
            </div>
            <form onSubmit={handleIntakeSubmit} className="space-y-3">
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Receiving Practitioner Name</label>
                <input
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Staff Badge ID</label>
                <input
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-white"
                  value={nurseBadge}
              ...
              <div className="bg-slate-900 p-2.5 rounded-xl border border-slate-800 text-[11px] text-slate-300 font-mono space-y-1">
